...
def raw_from_api_to_s3():
    @task(task_id="get_dates")
    def get_dates(data_interval_start=None, data_interval_end=None) -> tuple[str, str]:
        start_date = data_interval_start.format("YYYY-MM-DD")
        end_date = data_interval_end.format("YYYY-MM-DD")
        return start_date, end_date

    @task(task_id="get_and_transfer_api_data_to_s3")
    def get_and_transfer_api_data_to_s3(dates: tuple[str, str]) -> None:
        start_date, end_date = dates
        logging.info("💻 Start load for dates: %s/%s", start_date, end_date)
        ...
        logging.info("✅ Download for date success: %s", start_date)

    get_and_transfer_api_data_to_s3(get_dates())
